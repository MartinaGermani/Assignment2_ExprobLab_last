...
As you can see, initially the robot detects the aruco markers places around it by moving the camera by using some preconfigurated motion which I set using MoveIt framework. Once all the markers have been detected, their codes are translated in environment's information in order to build the semantic map of the environment which initially is empty. 
Once loaded the map, the robot is ready to start the patrolling. So, the reason state is called, in which the next location to be visisted is computed on the basis of the urgency and on the reachability. Once decided the target location, this information is sent to the planner, which plan the path from the current position to the target one and it responses with a list of via_points. These via_points are requested by the controller which controls each time the position of the robot in order to navigate it to the goal. Once the target location is reached, the robot performs a detection of the location itself by rotating the base of the robot.
Moreover, if the battery becomes low, the robot has to move in the DOC-station, placed at room E before to start the recharge. 
